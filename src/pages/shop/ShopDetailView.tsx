...
export function ShopDetailView({
  ...
  buyResult, popupCardId, detailLoading, user, isEn, productMode,
  onBack, onBuyPack, onBuyDisplay, onBuyStarter,
  ...
  const maxSlots = isStarter ? 3 : (productMode === 'display' ? 5 : 3);

  // Fallback: pick random cards from the set when no showcase cards are configured
  const showcaseFallback = useMemo(() => {
    const sourceIds = productMode === 'display'
      ? set.displayShowcaseCardIds
      : set.showcaseCardIds;
    if (sourceIds && sourceIds.length > 0) return sourceIds;
    if (!cards || cards.length === 0) return [];
    const count = productMode === 'display' ? 5 : 3;
    const shuffled = [...cards].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, count).map((c) => c.cardId);
  }, [set.showcaseCardIds, set.displayShowcaseCardIds, cards, productMode]);

  const startEditing = useCallback(() => {
    if (productMode === 'display') {
      setEditCards(set.displayShowcaseCardIds ?? []);
      setEditAnimated(set.displayShowcaseAnimated ?? false);
    } else {
      setEditCards(set.showcaseCardIds ?? []);
      setEditAnimated(set.showcaseAnimated ?? false);
    }
    setEditingShowcase(true);
  }, [set.showcaseCardIds, set.showcaseAnimated, set.displayShowcaseCardIds, set.displayShowcaseAnimated, productMode]);
    ...
    try {
      if (productMode === 'display') {
        await updateSetConfig(token, set.setName, {
          display_showcase_card_ids: editCards.length > 0 ? editCards : null,
          display_showcase_animated: editAnimated,
        });
        // Update local state so preview refreshes
        set.displayShowcaseCardIds = editCards.length > 0 ? editCards : null as any;
        set.displayShowcaseAnimated = editAnimated;
      } else {
        await updateSetConfig(token, set.setName, {
  ...
  }, [token, set, editCards, editAnimated, productMode]);
    ...
    <div className={styles.page}>
        ...
        <div className={`${cardStyles.card} ${set.active ? cardStyles.cardActive : cardStyles.cardInactive} ${styles.detailCard}`}>
          <div className={`${cardStyles.cardImage} ${set.active ? cardStyles.cardImageActive : cardStyles.cardImageInactive}`}>
            <SetShowcase
              ...
              code={set.code ?? 'N/A'}
              productType={isStarter ? 'starter' : productMode}
              animated={editingShowcase ? editAnimated : (productMode === 'display' ? (set.displayShowcaseAnimated ?? false) : set.showcaseAnimated)}
            />
              ...
              <span className={cardStyles.inactiveTag}>
                {set.gameReleaseDate
                  ? t('shop.availableFrom', {
                      date: new Date(set.gameReleaseDate).toLocaleDateString(isEn ? 'en-US' : 'de-DE'),
                    })
                  : t('shop.notAvailable')}
              </span>
            )}
          ...
          <div className={cardStyles.cardBody}>
            ...
            <div className={cardStyles.cardFooter}>
              <span className={cardStyles.cardPrice}>{productMode === 'display' ? set.priceDisplay : set.pricePack} DP</span>
        ...
        <div className={styles.heroInfo}>
          ...
          <div className={styles.statsRow}>
            ...
            {set.displaySize && (
              <div className={styles.statBox}>
                ...
                <span className={styles.statLabel}>{t('shop.packsPerDisplay')}</span>
              </div>
            )}
          </div>
          ...
          <div className={styles.buyRow}>
            {isStarter ? (
              ...
              <button className={`${styles.buyBtn} ${styles.buyBtnDisplay}`} onClick={onBuyDisplay} disabled={!canBuyDisplay || buying}>
      ...
      {isAdmin && (
        <div className={styles.showcaseAdminRow}>
          ...
          <button
            className={`${styles.buyBtn} ${!(editingShowcase ? editAnimated : set.showcaseAnimated) ? '' : styles.buyBtnDisplay}`}
